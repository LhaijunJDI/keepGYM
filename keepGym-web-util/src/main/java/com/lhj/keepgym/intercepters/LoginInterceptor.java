...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!methodAnnotation.isManager()) {
            //从浏览器中获取oldToken的值，若之前并未登录过则为空
            String token = CookieUtil.getCookieValue(request, "oldMembersToken", true);
            //从登录校验返回的数据中获取token
            ...
            if (!StringUtils.isEmpty(token)) {
                ...
                CookieUtil.setCookie(request, response, "oldMembersToken", token, 60 * 60 * 2, true);
                return true;
            }
        ...
        //从浏览器中获取oldToken的值，若之前并未登录过则为空
        String token = CookieUtil.getCookieValue(request, "oldManagerToken", true);
        ...
        if (!StringUtils.isEmpty(token)) {
            ...
            assert resultMap != null;
            result = resultMap.get("status");
            request.setAttribute("managerId", resultMap.get("managerId"));
            request.setAttribute("managerName", resultMap.get("managerName"));
            ...
            //map中状态为success，则将token保存在浏览器的cookie中并放行
            CookieUtil.setCookie(request, response, "oldManagerToken", token, 60 * 60 * 2, true);
            return true;
        }
        request.setAttribute("message", "您还没有登录，请登录后访问！");
        request.getRequestDispatcher("/login").forward(request, response);
